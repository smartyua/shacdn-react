...
Документ для **выбора нужного контроля** из папки `src/components/` репозитория shacdn. Каталог охватывает **50+ примитивов** в паритете с реестром [shadcn/ui](https://ui.shadcn.com) (без Tailwind: те же типичные высоты контролов **h-8 / h-9 / h-10**). Полный список папок см. навигацию в демо `App.tsx` и блок ниже. Перенос: [STYLE_GUIDE.md](./STYLE_GUIDE.md).
