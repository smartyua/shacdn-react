...
} from '../../components/PasswordToggleField/PasswordToggleField';
import {
  Toolbar,
  ToolbarButton,
  ToolbarLink,
...
const COMBO_FRAMEWORKS = ['Next.js', 'Remix', 'Astro', 'Nuxt.js', 'SvelteKit', 'SolidStart'];
...
const ComboboxSection = () => {
  ...
  return (
    <section id="combobox" className={styles.section}>
      <h2>Combobox</h2>
      <div className={styles.demoStack} style={{ maxWidth: '18rem' }}>
        <div>
          <p className={styles.sectionNote}>Legacy options API</p>
          <Combobox
        ...
        <div>
          <p className={styles.sectionNote}>Compound · clearable · soft</p>
          <Combobox
            items={COMBO_FRAMEWORKS}
            value={framework}
          ...
          >
            <ComboboxInput
              ...
              startSlot={<Search size={16} aria-hidden />}
            />
            <ComboboxContent>
        ...
        <div>
          <p className={styles.sectionNote}>Sizes</p>
          <div className={styles.demoStack}>
            {(['sm', 'md', 'lg'] as const).map(size => (
              <Combobox key={size} items={COMBO_FRAMEWORKS} size={size} icon>
                <ComboboxInput placeholder={`Size ${size}`} aria-label={`Size ${size}`} />
                <ComboboxContent>
                  <ComboboxEmpty>No frameworks found.</ComboboxEmpty>
                  <ComboboxList>
                    {(item: string) => (
...
const MentionTextareaDemo = () => {
  const [value, setValue] = useState(
    'Hey @grace look at this www.cursor.com check this out!!'
  );
  ...
  return (
    <div className={styles.demoStack}>
      <MentionTextarea
        aria-label="Mention composer"
        placeholder="Write a note… try @ada, /shrug, or paste a URL"
        rows={5}
        value={value}
        ...
        users={[
          ...
          { id: '3', label: 'Alan Turing', handle: 'alan' },
        ]}
        commands={[
          { id: 'shrug', name: 'shrug', description: 'Append ¯\\_(ツ)_/¯' },
          { id: 'table', name: 'tableflip', description: 'Append (╯°□°)╯︵ ┻━┻' },
          { id: 'clear', name: 'clear', description: 'Clear the composer' },
        ]}
        onCommand={command => {
...
export const ComponentsShowcase = () => {
  ...
  const [activeId, setActiveId] = useState(
    () => COMPONENTS_NAV[0]?.toLowerCase().replace(/ /g, '-') ?? ''
  );

  useEffect(() => {
    const sections = COMPONENTS_NAV.map(comp => {
      const id = comp.toLowerCase().replace(/ /g, '-');
    ...
    <div className={styles.wrapper}>
      <nav className={styles.navigation} aria-label="Components">
        <div className={styles.navBrand}>
          <h3 className={styles.navTitle}>Components</h3>
          <span className={styles.navBadge}>{COMPONENTS_NAV.length}</span>
        </div>
        <ul className={styles.navList}>
          {COMPONENTS_NAV.map(comp => {
            ...
            return (
              ...
              >
                <a href={`#${id}`} data-active={activeId === id || undefined}>
                  {comp}
                </a>
              </li>
            );
      ...
      <div className={styles.container}>
        <header className={styles.header}>
          <div className={styles.headerInner}>
            <div>
              ...
              <p>
                Production-ready primitives for humans and AI agents — SCSS modules, no Tailwind.
                Design parity with{' '}
                <a href="https://ui.shadcn.com" target="_blank" rel="noopener noreferrer">
                  ui.shadcn.com
                </a>
                ; docs chrome inspired by{' '}
                <a href="https://appica.dev/ui/" target="_blank" rel="noopener noreferrer">
                  Appica UI
                </a>
                .
              </p>
            ...
            <div className={styles.headerMeta}>
              <span className={styles.chip}>{COMPONENTS_NAV.length} components</span>
        ...
        <section id="password-toggle-field" className={styles.section}>
          <h2>Password Toggle Field</h2>
          <div style={{ maxWidth: '20rem' }}>
            <PasswordToggleField>
              <PasswordToggleFieldInput placeholder="Password" aria-label="Password" />
              <PasswordToggleFieldToggle>
                <PasswordToggleFieldIcon visible={<EyeOff />} hidden={<Eye />} />
              </PasswordToggleFieldToggle>
            </PasswordToggleField>
          </div>
        </section>
        ...
        <section id="mention-textarea" className={styles.section}>
          <h2>MentionTextarea</h2>
          <p>
            Type <code>@</code> to mention, <code>/</code> at line start for commands. URLs render as short
            link chips.
          </p>
          <MentionTextareaDemo />
        </section>
        ...
        <section id="toolbar" className={styles.section}>
          ...
          <Toolbar aria-label="Text formatting">
            ...
            <ToolbarSeparator />
            <ToolbarLink href="#toolbar">Link</ToolbarLink>
            <ToolbarSeparator />
